...
def recursiveSolve(x,y,width,height,visited,correctPath):
    endX = width-1
    endY = height-1
    ...
    if maze[x][y] == visited[x][y]:
        return correctPath
    if x != 0:
        if (recursiveSolve(x-1, y, width, height, visited, correctPath)):
    ...
    if x != width - 1:
        if (recursiveSolve(x+1, y, width, height, visited, correctPath)):
            ...
            return correctPath
    if y != 0:
        if (recursiveSolve(x, y-1, width, height, visited, correctPath)):
            correctPath[x][y] = True
            return correctPath
    if y != height - 1:
        if (recursiveSolve(x, y+1, width, height, visited, correctPath)):
...
def generate_maze(MAZE_HEIGHT, MAZE_WIDTH, maze, walls, cells, all_sprites, midX, midY):
    ...
    correctPath = recursiveSolve(0,0,MAZE_HEIGHT,MAZE_WIDTH,visited,correctPath)
    ...
    for x in range(0, MAZE_HEIGHT):
        for y in range(0, MAZE_WIDTH): #set up a nested FOR loop for the maze columns
            X = (MAZE_CONSTANT*x)+(midX-((MAZE_CONSTANT*MAZE_WIDTH) / 2)) #set the X coordinate to be the pixel co-ordinate for the sprite
            Y = (MAZE_CONSTANT*y)+(midY-(MAZE_CONSTANT*MAZE_HEIGHT)) #set the Y coordinate to be the pixel co-ordinate for the sprite
...
class pygame_window(screens):
    ...
    def display(self, qs_diff):
        ...
        keys, visited, self.__maze = generate_maze(self.__maze_height, self.__maze_width, self.__maze,
                            self.__walls, self.__cells, self._all_sprites, self.__midX, self.__midY)
        ...
        while run:
            ...
            if self.__movesLeft <= 0:
                if self.calls < amountOfQs:
                    self.open_question(questions) #calls the 'open_question' method of 'pygame_window'
            ...
            if hintRevealed: #checks if a hint has been revealed
                hint = revealHint(visited, self.__maze) #if it has, then calls the revealHint function
...
class Game(object):
    ...
    def run_loading(self, qs_diff):
        self.display8.display(qs_diff) #call the display method of the class assigned to 'display7'

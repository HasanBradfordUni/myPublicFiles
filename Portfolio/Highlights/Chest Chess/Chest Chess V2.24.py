...
class Pawn(Piece):
    ...
    def get_possibleMoves(self, piecePos, possibleMoves, moveNum1, moveNum2, capturing):
        pieceX = int(((piecePos[0] - 200) / 100) - 1)
        pieceY = int(((piecePos[1] - 100) / 100) - 1)
        print(pieceX,pieceY) 
    ...
    def checkMove(self, moveFrom, moveWhere, possibleMoves, capturing):
        X = moveWhere[0]
        Y = moveWhere[1]
        int(pieceX = ((X - 200) / 100) - 1)
        int(pieceY = ((Y - 100) / 100) - 1)
            ...
            if clicks % 2 == 0:
                ...
                if not gameOver:
                    ...
                    for piece in pieces:
                        ...
                        coordsX = coords[0]
                        coordsY = coords[1]
                        if (coordsX - 50) < mouseX < (coordsX + 50) or (coordsY - 50) < mouseY < (coordsY + 50):
                            thisPiece = piece
                    print(origPossibleMoves)
                    possibleMoves = origPossibleMoves
                    possibleMoves = thisPiece.get_possibleMoves(coords, possibleMoves, moveNum1, moveNum2, False)

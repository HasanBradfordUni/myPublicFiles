...
def insertion_statement(templ_statement, data_table):
    ...
    for i in range(1, len(first_row) + 1):
        if first_row[i - 1].isdigit():
            actual_statement = templ_statement.replace(f"'[value-{i}]'", first_row[i - 1])
        else:
            actual_statement = templ_statement.replace(f"[value-{i}]", first_row[i - 1])
    ...
    WHERE column_name IN(‘column_name1’, ‘column_name2’)"""
        ...
        for i in range(0, len(data_table) + 1):
            if data_table[i][0].isdigit():
                edited_statement = actual_statement.replace(f"'[column_name{i}]'", data_table[i][0])
            else:
                edited_statement = actual_statement.replace(f"[column_name{i}]", data_table[i][0])
            if data_table[i][1].isdigit():
                actual_statement = edited_statement.replace(f"'[column_value{i}]'", data_table[i][1])
            else:
                actual_statement = edited_statement.replace(f"[column_value{i}]", data_table[i][1])

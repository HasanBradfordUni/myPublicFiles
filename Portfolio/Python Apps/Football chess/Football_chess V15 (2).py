...
class Player():
    def __init__(self, playerName, score, playerTurn):
...
def checkMovePath(array, moveFromX, moveFromY, moveToX, moveToY):
    ...
    if moveDif[0] == 0:
        if stepX > 0:
            for x in range(moveFromY+1, moveFromY+moveDif[1], stepX):
                if array[moveFromX][x] != None:
                    pathClear = False
                    break
        else:
            for x in range(moveFromY, moveFromY+moveDif[1]+1, stepX):
                if array[moveFromX][x-1] != None:
                    pathClear = False
                    break
    elif moveDif[1] == 0:
        if stepY > 0:
            for y in range(moveFromX+1, moveFromX+moveDif[0], stepY):
                if array[y][moveFromY] != None:
                    pathClear = False
                    break
        else:
            for y in range(moveFromX, moveFromX+moveDif[0]+1, stepY):
                if array[y-1][moveFromY] != None:
                    pathClear = False
                    break
    else:
...
def move(moveFrom, moveTo, array, player1Turn, player2Turn):
    ...
    if moveValid and pathClear:
        if piece2 != None:
            if player1Turn and piece2.lower() == piece2:
                moveValid = False
                print("\nYou cannot capture your own piece!\n")
            elif player2Turn and piece2.upper() == piece2:
                moveValid = False
                print("\nYou cannot capture your own piece!\n")
        if moveValid:
        ...
        if piece2 != None:
            if player1Turn and piece2.upper() == piece2:
                player1Capture = True
            elif player2Turn and piece2.lower() == piece2:
                player2Capture = True
            piece2 = piece2.upper()
    elif moveValid and not pathClear:
        print("\nThe path is not clear - you cannot make this move!\n")
        moveValid = False
    else:
...
def CheckGK(piecesArray, player1Turn, player2Turn):
    if player1Turn:
        if piecesArray[0][3] == "gk" or piecesArray[0][4] == "gk":
            return True
        else:
            return False
    elif player2Turn:
        if piecesArray[7][3] == "GK" or piecesArray[7][4] == "GK":
            return True
        else:
            return False
    else:
...
def checkGameOver(piecesArray, player1Turn, player2Turn):
    gameOver = True
    for array in piecesArray:
        for piece in array:
            if piece == "gk" or piece == "GK":
                gameOver = False
    return gameOver

def CalculateWinner(player1Score, player2Score, player1, player2):
    if player1Score > player2Score:
        winner = player1.get_playerName()
    elif player2Score > player1Score:
        winner = player2.get_playerName()
    else:
        winner = "Draw!"
    return winner
...
def new_game():
    ...
    if turnNum == 1:
        ...
        player2Turn = True
    player1 = Player("Player1", 0, player1Turn)
    player2 = Player("Player2", 0, player2Turn)
    ...
    while not gameOver:
        if player1Turn:
            print("\nPlayer 1 turn!\n")
            moveFrom, moveTo, gameOver2 = player1.get_move()
            if not gameOver2:
                ...
                while not moveValid:
                    ...
                    while not moveValid1:
                        ...
                        while not moveValid2:
                            ...
                            gkInGoal = CheckGK(array, player1Turn, player2Turn)
                            if gkInGoal:
                                player1.set_score(player1Score+5)
                            player1Score = player1.get_score()
                            if player1Capture:
                ...
                quit()
            gameOver = checkGameOver(array, player1Turn, player2Turn)
            ...
            print("\nPlayer 2 turn!\n")
            moveFrom, moveTo, gameOver2 = player2.get_move()
            if not gameOver2:
                ...
                while not moveValid:
                    ...
                    while not moveValid1:
                        ...
                        while not moveValid2:
                            ...
                            gkInGoal = CheckGK(array, player1Turn, player2Turn)
                            if gkInGoal:
                                player2.set_score(player2Score+5)
                            player2Score = player2.get_score()
                            if player2Capture:
         ...
         winner = CalculateWinner(player1Score, player2Score, player1, player2)
...
def how_to_play():
    ...
    print("The game will end when both players' goalkeeper gets captured")

...
def generate_maze(MAZE_HEIGHT, MAZE_WIDTH, maze, walls, cells, all_sprites, midX, midY):
    ...
    correctPath = recursiveSolve(0,0)
        ...
        for y in range(0, MAZE_WIDTH): #set up a nested FOR loop for the maze columns
            ...
            if maze[x][y] == False: #check if maze index is a wall
                ...
                if x == (MAZE_WIDTH-1): #if it is the exit of the maze
                    ...
                    return keys, visited, correctPath #returns the 'keys' group to the 'pygame_window' class and also visited and correctPath 2D arrays  

def revealHint(visited, correctPath):
    hint = (0, 0)
    for x in range(len(visited)):
        for y in range(len(correctPath)):
            if (visited[x][y] and visited[x+1][y]) and (visited[x][y] and visited[x][y+1]):
                if correctPath[x+1][y]:
                    hint = (x+1, y)
                elif correctPath[x][y+1]:
                    hint = (x, y+1)
    return hint
...
class pygame_window(screens):
    ...
    def display(self, qs_diff):
        ...
        keys, visited, correctPath = generate_maze(self.__maze_height, self.__maze_width, self.__maze,

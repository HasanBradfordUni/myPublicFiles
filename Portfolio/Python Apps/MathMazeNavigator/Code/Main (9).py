...
from Information_objects import Scoreboard, LivesC, MovesC, Timer, SaveButton, LoadButton
...
from QS_objects import Text, Button1, Button2, Button3, Button4, Button5
from Maths_quiz import Window, Results
...
class QuestionSelect_screen(screens):
    def __init__(self):
        ...
        self.__SyQD = Text((self.__locX1,self.__locY1),"Select your Question Difficulty")
...
class high_scores_screen2(Toplevel):
    def __init__(self, parent, score, name):
        ...
        for num in range(1, len(sortedNames)+1): #uses a FOR loop to iterate over over the sortedNames array
            ...
            textBox.insert(END, text) #inserts the text into the textBox

        #set up a button which closes the window
        Button(self,text='Close',bg="#12AAE6",command=self.destroy).grid(row=2, columnspan=2)
...
class high_scores_screen1(Tk):
    def __init__(self, score):
        ...
        self.textBox = Text(self, bg="#12AAE6")
        self.textBox.grid(row=2, columnspan=2)

        #set up a button which opens the new window for the high scores
        Button(self,text='Submit score',bg="#12AAE6",command=lambda: self.open_window(score)).grid(row=3, column=0)
        #set up a button which closes the window
    ...
    def open_window(self, score):
        ...
        if len(name) > 10 or len(name) < 2: #check if the name is valid
            ...
            hss2 = high_scores_screen2(self, score, name) #calls the Tkinter window 'high_scores_screen2'
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN: #checks if a keyboard key is pressed
                    if event.key == pygame.K_f: #if the key is f
                        pygame.display.toggle_fullscreen() #toggles fullscreen mode of the pygame window
                if event.type == pygame.MOUSEBUTTONDOWN:
                    hss1 = high_scores_screen1(currentScore) #calls the Tkinter window 'high_scores_screen1'
...
class pygame_window(screens):
    ...
    def display(self, qs_diff):
        ...
        while run:
            ...
            #calls the maths question if 'movesLeft' is 0 or less
            if self.__movesLeft <= 0:
                self.open_question(score, questions, calls)
            ...
            if pygame.sprite.spritecollideany(self.__player, keys):
                self.__CurrentScore = 1500 - (2.5 * self.__moves) #if it does calculate the score
                ...
                game.run_win(self.__CurrentScore) #run the win screen from its display method in game
    ...
    def open_question(self, score, questions, calls):
        calls += 1 #increment calls by 1
        if calls < 10: #check if calls is less than 10
            window = Window(calls, questions) #if it is, calls itself for next question
            thisQuestion = questions[calls-1]
            question = thisQuestion[0]
            ...
            results = Results(score) #calls the results window
...
class Game(object):
    ...
    def run_win(self, currentScore):
        self.display5.display(currentScore) #call the display method of the class assigned to 'display5'

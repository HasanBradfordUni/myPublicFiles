...
class Window(Toplevel):
    def __init__(self, parent, query, pageNum, searchResults, total):
        ...
        Label(self, text=f"Showing 10 results of {total}", bg="Red").grid(row=3, columnspan=2)
        
        Button(self,text="Next Page",bg="Red",command=lambda: self.next_page(query, total)).grid(row=4, column=0)
        Button(self,text='Close',bg="Red",command=self.destroy).grid(row=4, column=1)

    def next_page(self, query, total):
        global pageNum
        pageNum += 1
        searchResults, total = search(query, pageNum)
        window = Window(self, query, pageNum, searchResults, total)
        window.grab_set()
...
class App(Tk):
    def __init__(self, pageNum):
        ...
        Button(self,text='Submit',bg="Red",command=lambda: self.open_search(pageNum)).grid(row=3, column=0)
        Button(self,text='Exit',bg="Red",command=self.destroy).grid(row=3, column=1)

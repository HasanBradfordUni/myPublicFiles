...
class Dastan:
    ...
    def __CountNormalPieces(self):
        ...
        for S in self._Board:
            PieceInSquare = S.GetPieceInSquare()
            if PieceInSquare is not None:
                if PieceInSquare.GetBelongsTo().SameAs(self._CurrentPlayer) and PieceInSquare.GetTypeOfPiece() == "piece":
                    pieces += 1
        return pieces

    def __CheckReincarnation(self, SquareReference):
        Row = SquareReference // 10
        Col = SquareReference % 10
        if Row == self._NoOfRows and self._CurrentPlayer.GetName() == "Player One":
            print("Congratulations! You have reached the end of the board")
            if self.__CountNormalPieces() < self._NoOfPieces:
                print("You have less pieces than normal, therefore you deserve a reincarnation!")
                SquareIsValid = False
                while not SquareIsValid:
                    ReincarnationColumn = int(input("Enter the column number to reincarnate a piece onto: "))
                    if 0 < ReincarnationColumn < self._NoOfColumns + 1:
                        if self._Board[self.__GetIndexOfSquare(10 + ReincarnationColumn)].GetPieceInSquare() == None:
                            print("The reincarnation is valid!")
                            SquareIsValid = True
                        else:
                            SquareIsValid = False
                    else:
                        SquareIsValid = False
                if SquareIsValid:
                    self._Board[self.__GetIndexOfSquare(10 + ReincarnationColumn)].SetPiece(Piece("piece", self._CurrentPlayer, 1, "!"))
        elif Row == 1 and self._CurrentPlayer.GetName() == "Player Two":
            print("Congratulations! You have reached the end of the board")
            if self.__CountNormalPieces() < self._NoOfPieces:
                print("You have less pieces than normal, therefore you deserve a reincarnation!")
                SquareIsValid = False
                while not SquareIsValid:
                    ReincarnationColumn = int(input("Enter the column number to reincarnate a piece onto: "))
                    if 0 < ReincarnationColumn < self._NoOfColumns + 1:
                        if self._Board[self.__GetIndexOfSquare(60 + ReincarnationColumn)].GetPieceInSquare() == None:
                            print("The reincarnation is valid!")
                            SquareIsValid = True
                        else:
                            SquareIsValid = False
                        ...
                        SquareIsValid = False
                if SquareIsValid:
                    self._Board[self.__GetIndexOfSquare(60 + ReincarnationColumn)].SetPiece(Piece("piece", self._CurrentPlayer, 1, '"'))

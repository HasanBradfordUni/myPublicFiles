...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        ...
        self._weatherEventCreated = False
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                ...
                if Choice == 9:
                    ...
                    self.__DisplayState()
                self._weatherEventCreated = self.__WeatherEventOccurs()
                if self._weatherEventCreated:
                    countDownCompleted = self._weatherEvent.CountDownComplete()
                    if countDownCompleted:
                        location = self._weatherEvent.GetWeatherLocation()
                        col = location % 10
                        for Row in range(1, self._NoOfRows + 1):
                            for Column in range(1, self._NoOfColumns + 1):
                                Index = self.__GetIndexOfSquare(Row * 10 + Column)
                                if Column == col:
                                    self._Board[Index].RemovePiece()
                        self._weatherEventCreated = False
    ...
    def __WeatherEventOccurs(self):
        ...
        if chance == 1:
            if self._weatherEventCreated:
                eventCreated = True
                countDown = self._weatherEvent.GetCountDown()
                if countDown > 0:
                    print("The game tried to a create another Weather event!")
                elif countDown == 0:
                    print("Weather event will now occur!")
                    row = random.randint(1, self._NoOfRows)
                    col = random.randint(1, self._NoOfColumns)
                    location = row * 10 + col
                    self._weatherEvent.SetWeatherLocation(location)
            else:
                self._weatherEvent = WeatherEvent(2)
                print("A Weather Event has been created - it will occur in 2 turns")
                eventCreated = True
        return eventCreated
...
class WeatherEvent:
    ...
    def CountDownComplete(self):
        if self.__Countdown == 0:
            ...
            print("The weather event will occur in",self.__Countdown,"turns!")
            self.__Countdown -= 1
            return False

    def SetWeatherLocation(self, location):
